...
def djikstratarget(start_y, start_x, target_y, target_x):
    ...
    d, p, l = g.djikstra(start, target)
    if d <= 0 or p is None:
        return Response(status_code=204)
    s = geopandas.GeoDataFrame({"cost": [d], "distance": [l], 'geometry': [shapely.LineString([wierzcholki[node][:2] for node in p])]}, crs="EPSG:2180")
...
def a_star(start_y, start_x, target_y, target_x):
    ...
    s = geopandas.GeoDataFrame({"cost": [d], "distance": [l], 'geometry': [shapely.LineString([wierzcholki[node][:2] for node in p])]}, crs="EPSG:2180")
    return Response(content=s.to_json(), media_type="application/json")
@ app.get("/djikstrarange/{start_y}/{start_x}/{max_cost}/{buffer}/{step}")
def djikstrarange(start_y, start_x, max_cost, buffer, step):
